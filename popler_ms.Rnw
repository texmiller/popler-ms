...
\subsection*{Creation of the \texttt{popler} database}
...
The \texttt{popler} database resulting from this process contains data from \Sexpr{length(dictionary(proj_metadata_key)$proj_metadata_key)} studies (\Sexpr{nrow(browse(studytype == "exp"))} of which are experimental), \Sexpr{sum(browse()$duration_years,na.rm=T)} years %of observations measured over ## spatial replicates. 
\texttt{popler} contains data from \Sexpr{length(dictionary(species)$species)} species, of which 691 are plants, 339 are animals, and only one is a fungi.  
...
dictionary()
dictionary(title)
length(dictionary(proj_metadata_key)$proj_metadata_key)
dictionary(duration_years)
dictionary(lat_lter)
nrow(browse(studytype == "exp"))
names(browse(full_tbl=T))
browse(full_tbl=T)$n_spat_levs
browse(taxas)

length(dictionary(species)$species)
