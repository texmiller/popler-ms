...
\newpage
\begin{table}[h!]
  \caption{LTER identification acronyms and their meaning as used in the \texttt{popler} database. \aldo{These are not 28: I need to update list by final draft} }
  \label{Tab:S3}
  \begin{center}
    \begin{tabular}{l l}
      \hline
